...
public class SkillShot2 : MonoBehaviour
{
    Animator anima;
    private GameObject Skill;   // переменная для создания скила
    private GameObject Player2;  // переменная для определения позиции игрока
    ...
    void Start()
    {
        Player2 = GameObject.Find("Player2"); // отпределение игрока
    }
    ...
    public void SkillLaunch(string skill_name)
    {
        ...
        movVect = Player2.GetComponent<Animator>(); // направление игрока

        x = movVect.GetFloat("input_x"); // поззиция Х
        y = movVect.GetFloat("input_y"); // позиция У

		
        Skill.transform.position = Player2.transform.position; // позиция игрока
        ...
        if (skill_name == "lightning")
        {
            ...
            {
                Skill.transform.position = Player2.transform.position + new Vector3(LightningDistance, 0f, 0f);
		...
		if (skill_name == "heal")
        {
            Player2.GetComponent<HP>().Heal(heal_amount);
            Destroy(Skill, 2f);
        }
    ...
    void Update()
    {
        ...
        direction = mobs.transform.position - Player2.transform.position; //вычисляем расстояние между ним и нашим игроком
        if (Input.GetKeyDown(KeyCode.Keypad1)) //&& direction.x >= 40f)
        { //если нажата кнопка и расстояние больше 40, запускает скилл
            if (skills[0].currentCoolDown >= skills[0].cooldown)
            {
                ...
                {
                    Skill.GetComponent<Rigidbody2D>().AddForce(Player2.transform.up * Speed, ForceMode2D.Impulse);
                }
                ...
                {
                    Skill.GetComponent<Rigidbody2D>().AddForce(Player2.transform.right * Speed, ForceMode2D.Impulse);
                }
        ...
        {
            Skill.transform.position = Player2.transform.position;
        }
		
        foreach (Skills2 s in skills)
    ...
    IEnumerator Haste()
    {
        Player2.GetComponent<PlayerMovement2>().moveSpeed += 0.5f;
