...
public class StorageInventory : MonoBehaviour
{
	...
	public int distanceToOpenStoragePlayer2;
	...
	public GameObject destroyStorage;
    bool closeInv;
    ...
    void Update()
    {
		...
		if (player2 != null) {
			...
			float distanceP2 = Vector2.Distance (this.gameObject.transform.position, player2.transform.position);
			...
			if (distanceP2 <= distanceToOpenStorage && Input.GetKeyDown (inputManagerDatabase.StorageKeyCode2)) {
				showStorage = !showStorage;
				StartCoroutine (OpenInventoryWithTimer ());
			}

			if (distance > distanceToOpenStorage && showStorage && distanceP2 > distanceToOpenStorage) {
    ...
    IEnumerator OpenInventoryWithTimer()
    {
        if (showStorage)
        {
            ...
            if (showStorage)
            {
				...
				storageItems.Clear();
                showTimer = false;
				Destroy (destroyStorage); //удалить сундук
                if (timer != null)
                    timer.SetActive(false);
            }
        }
